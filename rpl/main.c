    ...
    while (1)
    {
        ...
        if (msg.type == GNRC_NETAPI_MSG_TYPE_SND)
        {
            ...
            for (unsigned i = 0; i < (res / sizeof(ipv6_addr_t)); i++)
            {
                ...
                if ((!ipv6_addr_is_link_local(&addrs[i])) && (!ipv6_addr_is_link_local(&ipv6_hdr->src)) && (!ipv6_addr_is_link_local(&ipv6_hdr->dst)) && (!ipv6_addr_equal(&addrs[i], &(ipv6_hdr->src))))
                {
                    char addr_str[IPV6_ADDR_MAX_STR_LEN];
                    printf("IPv6 ROUTER: forward from src = %s to dst = %s", ipv6_addr_to_str(addr_str, &(ipv6_hdr->src), sizeof(addr_str)), ipv6_addr_to_str(addr_str, &(ipv6_hdr->dst), sizeof(addr_str)));
                }
            }
        }
